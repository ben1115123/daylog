...
- `db.js` → `syncToAppleCalendar(action, event)` is called (non-blocking) after `addEvent`/before `deleteEvent`. On successful add, `events.apple_uid` is updated with the returned resource URL so delete can target it later.
- **Failures are surfaced, not swallowed.** The error message is `console.error`'d and written to `events.apple_sync_error` (cleared on the next success); a `daylog:sync` window event carries `{ id, error }` so Home and Calendar can patch the row in place without a refetch. Failed events show a muted `SyncFailedIcon` — `.pill-sync-failed` on the Home upcoming pill, `.ev-sync-failed` on the Calendar row — in `--text3`, never `--red`: the entry itself saved fine. A failed `delete` has no row left to annotate, so the console line is its only record. Sync outcome never blocks or fails the save.
